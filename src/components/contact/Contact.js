...
function Contact() {
  ...
  const sendEmail = (e) => {
    ...
    emailjs.sendForm('service_414kml7', 'template_kqbh4yj', form.current, '-TF_ElUxPaTBeCWM4')
      .then((result) => {
                ...
                <form ref={form} onSubmit={sendEmail} className=' w-full flex flex-col gap-6 py-5'>
                    ...
                    <div className='flex flex-col gap-4'>
                        <p className=' text-sm text-gray-400 uppercase tracking-wide'>Email</p>
                            <input onChange={(e)=>setEmail(e.target.value)} value={email} email className={`${
                    ...
                    <div className='w-full'>
                        ...
                        type='submit'
                        onClick={handleSend}
                        className="w-full h-12 bg-[#141518] rounded-lg text-base text-gray-400 tracking-wider uppercase hover:text-white duration-300 hover:border-[1px] hover:border-designColor border-transparent"   value='Send'>
                            Send Message
                        </button>
                    </div>
                    {
